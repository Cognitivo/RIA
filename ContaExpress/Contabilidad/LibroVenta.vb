...
#Region "Methods"
    ...
    Private Sub LibroVenta_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub Libros_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            Me.PARAASENTARCAB_COMPRASTableAdapter.FillBy(Me.DsPlantillasConta.PARAASENTARCAB_COMPRAS, 8, 10, 29, FechaFiltro1, FechaFiltro2)
    ...
    Sub MostrarEncabezado()
        ...
        Dim TipoFactura, Modulo As Integer

        conexion = ser.Abrir()

        If dgvParaAsentar.CurrentRow.Cells("ModuloID").Value = 8 Then '----- VENTAS
            Modulo = 8

            objCommand.CommandText = "SELECT dbo.CLIENTES.NOMBRE AS CLIENTE, dbo.CLIENTES.RUC AS RUC, dbo.VENTAS.NUMVENTA AS NUMFACTURA, dbo.VENTAS.TIPOVENTA, " & _
                                     "ROUND(dbo.VENTAS.TOTALVENTA, 0) AS TOTALFACTURA, ROUND(dbo.VENTAS.TOTALGRAVADO5, 0) AS TOTALGRAVADO5, ROUND(dbo.VENTAS.TOTALGRAVADO10, 0) AS TOTALGRAVADO10, ROUND(dbo.VENTAS.TOTALGRAVADO5/21, 0) AS IVA5, " & _
                                     "ROUND(dbo.VENTAS.TOTALGRAVADO10/11, 0) AS IVA10, ROUND(dbo.VENTAS.TOTALIVA, 0) AS TOTALIVA, ROUND(dbo.VENTAS.TOTALEXENTA, 0) AS TOTALEXCENTA, dbo.VENTAS.CODCOMPROBANTE, " & _
                                     "dbo.MONEDA.DESMONEDA, dbo.VENTAS.CODMONEDA FROM  dbo.VENTAS INNER JOIN dbo.MONEDA ON dbo.VENTAS.CODMONEDA = dbo.MONEDA.CODMONEDA LEFT OUTER JOIN dbo.CLIENTES ON dbo.CLIENTES.CODCLIENTE = dbo.VENTAS.CODCLIENTE " & _
                                     "WHERE(dbo.VENTAS.CODVENTA = " & dgvParaAsentar.CurrentRow.Cells("RegistroID").Value & ")"
            ...
            If odrDatos.HasRows Then
                Do While odrDatos.Read()
                    ...
                    If IsDBNull(odrDatos("IVA5")) Then
                        tbxIva5.Text = ""
                    Else
                        tbxIva5.Text = FormatNumber(odrDatos("IVA5"), 0)
                    End If

                    If IsDBNull(odrDatos("IVA10")) Then
                        tbxIva10.Text = ""
                    Else
                        tbxIva10.Text = FormatNumber(odrDatos("IVA10"), 0)
                    End If
                    ...
                    If Not IsDBNull(odrDatos("CODMONEDA")) Then
                        tbxMoneda.Text = f.FuncionConsultaString("DESMONEDA", "MONEDA", "CODMONEDA", FormatNumber(odrDatos("CODMONEDA"), 0))
                    Else
        ...
        ElseIf dgvParaAsentar.CurrentRow.Cells("ModuloID").Value = 29 Then '----- DEVOLUCIONES DE COMPRAS
            Modulo = 29

            objCommand.CommandText = "SELECT dbo.PROVEEDOR.NOMBRE AS PROVEEDOR,dbo.PROVEEDOR.RUC_CIN as RUC,dbo.DEVOLUCIONCOMPRA.NUMDEVOLUCION AS NUMDEVOLUCION, " & _
                                    " dbo.COMPRAS.MODALIDADPAGO,dbo.DEVOLUCIONCOMPRA.TOTALDEVOLUCION AS TOTALDEVOLUCION,ROUND(dbo.DEVOLUCIONCOMPRA.TOTALIVA5 * 21,0) AS TOTALGRAVADO5, " & _
                                    " ROUND(dbo.DEVOLUCIONCOMPRA.TOTALIVA10 * 11,0) AS TOTALGRAVADO10,dbo.DEVOLUCIONCOMPRA.TOTALIVA,dbo.DEVOLUCIONCOMPRA.TOTALEXENTA, " & _
                                    " dbo.DEVOLUCIONCOMPRA.CODCOMPROBANTE,dbo.DEVOLUCIONCOMPRA.TOTALIVA5,dbo.DEVOLUCIONCOMPRA.TOTALIVA10,dbo.MONEDA.DESMONEDA FROM dbo.DEVOLUCIONCOMPRA Join dbo.MONEDA on dbo.Moneda.CODMONEDA=dbo.DEVOLUCIONCOMPRA.CODMONEDA " & _
                                    " LEFT OUTER JOIN dbo.PROVEEDOR ON dbo.DEVOLUCIONCOMPRA.CODPROVEEDOR=dbo.PROVEEDOR.CODPROVEEDOR  LEFT OUTER JOIN dbo.COMPRAS ON dbo.PROVEEDOR.CODPROVEEDOR=dbo.COMPRAS.CODPROVEEDOR " & _
                                    " WHERE(dbo.DEVOLUCIONCOMPRA.CODDEVOLUCION = " & dgvParaAsentar.CurrentRow.Cells("RegistroID").Value & ")"
            ...
            If odrDatos.HasRows Then
                Do While odrDatos.Read()
                    ...
                    If IsDBNull(odrDatos("DESMONEDA")) Then
                        tbxMoneda.Text = ""
                    Else
                        tbxMoneda.Text = odrDatos("DESMONEDA")
                    End If
        ...
        Else '----- COBROS
            Modulo = 10
            Dim TieneVenta As Integer = 0
            TieneVenta = f.FuncionConsultaDecimal("CODVENTA", "VENTASFORMACOBRO", "CABCOBRO", dgvParaAsentar.CurrentRow.Cells("RegistroID").Value)

            '##################################################ANTICIPO DE CLIENTES#################################################BY SAUL PAVON
            If TieneVenta = 0 Then
                objCommand.CommandText = "SELECT SUM(dbo.VENTASFORMACOBRO.IMPORTE) AS IMPORTE, dbo.VENTASFORMACOBRO.NRORECIBO, dbo.VENTASFORMACOBRO.TIPOCOBRO, " & _
                                                 "dbo.VENTASFORMACOBRO.CABCOBRO, dbo.VENTASFORMACOBRO.FECHACOBRO, dbo.CLIENTES.NOMBRE, dbo.CLIENTES.RUC " & _
                                                 "FROM  dbo.CLIENTES INNER JOIN dbo.VENTASFORMACOBRO ON dbo.CLIENTES.CODCLIENTE = dbo.VENTASFORMACOBRO.CODCLIENTECAB " & _
                                                 "GROUP BY dbo.VENTASFORMACOBRO.NRORECIBO, dbo.VENTASFORMACOBRO.TIPOCOBRO, dbo.VENTASFORMACOBRO.CABCOBRO, " & _
                                                 "dbo.VENTASFORMACOBRO.FECHACOBRO, dbo.CLIENTES.NOMBRE, dbo.CLIENTES.RUC " & _
                                                 "HAVING(dbo.VENTASFORMACOBRO.CABCOBRO = " & dgvParaAsentar.CurrentRow.Cells("RegistroID").Value & ")"
                objCommand.Connection = New SqlConnection(ser.CadenaConexion)
                objCommand.Connection.Open()
                odrDatos = objCommand.ExecuteReader()
            ...
            Else
                objCommand.CommandText = "SELECT SUM(dbo.VENTASFORMACOBRO.IMPORTE) AS IMPORTE, dbo.VENTASFORMACOBRO.NRORECIBO, dbo.VENTASFORMACOBRO.TIPOCOBRO,  " & _
                                           "dbo.VENTASFORMACOBRO.CABCOBRO, dbo.VENTASFORMACOBRO.FECHACOBRO, dbo.CLIENTES.NOMBRE, dbo.CLIENTES.RUC, dbo.VENTAS.TIPOVENTA, " & _
                                           "dbo.VENTAS.CODCOMPROBANTE FROM dbo.VENTASFORMACOBRO INNER JOIN dbo.VENTAS ON dbo.VENTASFORMACOBRO.CODVENTA = dbo.VENTAS.CODVENTA INNER JOIN " & _
                                           "dbo.CLIENTES ON dbo.VENTAS.CODCLIENTE = dbo.CLIENTES.CODCLIENTE " & _
                                           "GROUP BY dbo.VENTASFORMACOBRO.NRORECIBO, dbo.VENTASFORMACOBRO.TIPOCOBRO, dbo.VENTASFORMACOBRO.CABCOBRO, " & _
                                           "dbo.VENTASFORMACOBRO.FECHACOBRO, dbo.CLIENTES.NOMBRE, dbo.CLIENTES.RUC, dbo.VENTAS.TIPOVENTA, dbo.VENTAS.CODCOMPROBANTE " & _
                                           "HAVING(dbo.VENTASFORMACOBRO.CABCOBRO = " & dgvParaAsentar.CurrentRow.Cells("RegistroID").Value & ")"
                objCommand.Connection = New SqlConnection(ser.CadenaConexion)
                objCommand.Connection.Open()
                odrDatos = objCommand.ExecuteReader()
        ...
        TipoFactura = f.FuncionConsultaDecimal("MAX(TIPOPAGO)", "PARAASENTAR", "ModuloID = " & Modulo & " AND RegistroID", dgvParaAsentar.CurrentRow.Cells("RegistroID").Value)
    ...
    Private Sub lblVerModulo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lblVerModulo.Click
        ...
        Else
            If dgvParaAsentar.CurrentRow.Cells("ModuloID").Value = 8 Then
                VentasPlus.BuscarTextBox.Text = tbxComprobante.Text
                VentasPlus.Show()
                VentasPlus.VENTASTableAdapter.Fill(VentasPlus.DsFacturacionCompleja.VENTAS, "01/01/1900", "01/01/2020")
            ElseIf dgvParaAsentar.CurrentRow.Cells("ModuloID").Value = 11 Then
            ...
            ElseIf dgvParaAsentar.CurrentRow.Cells("ModuloID").Value = 10 Then
                CobroV2.BuscarTextBox.Text = tbxComprobante.Text
                CobroV2.Show()
                CobroV2.pnlFiltradoTipoCobro.BringToFront()
                CobroV2.FechaDesde3.Text = tbxFecha.Text
                CobroV2.FechaHasta3.Text = tbxFecha.Text
                CobroV2.btnFiltrarCuentas_Click(Nothing, Nothing)
            End If
        End If
    End Sub
    ...
    Private Sub btnAsentar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAsentar.Click
        Dim ModuloID, RegistroId As String
        ...
        If dgvParaAsentar.RowCount <> 0 Then
                ...
                Do While odrAsientos.Read()
                    ...
                    Try
                        consulta = "INSERT INTO ASIENTOS (ModuloID,RegistroID,CODMONEDA,FECHAASIENTO,COTIZACION, " & _
                                    ...
                                    ", " & odrAsientos("CODPERIODOFISCAL") & ", " & odrAsientos("CODPLANCUENTA") & ", '" & odrAsientos("DESCRIPCION") & "', " & _
                    ...
                    Catch ex As Exception
                        MessageBox.Show("Error: " & ex.Message, "POSEXPRESS")
            ...
            Try
                If dgvParaAsentar.CurrentRow.Cells("ModuloID").Value = 8 Then   'Ventas
                    ...
                    consulta = "UPDATE VENTAS SET ASENTADO = 2 WHERE CODVENTA = " & RegistroId & ""
                    myTrans = conexion.BeginTransaction(IsolationLevel.ReadCommitted, "Actualizar")

                    Consultas.ConsultaComando(myTrans, consulta)
                    myTrans.Commit()

                ElseIf dgvParaAsentar.CurrentRow.Cells("ModuloId").Value = 29 Then
                    consulta = "UPDATE DevolucionCompra SET ASENTADO = 2 WHERE CODDEVOLUCION = " & RegistroId & ""
                    myTrans = conexion.BeginTransaction(IsolationLevel.ReadCommitted, "Actualizar")
    ...
    Private Sub btnAgregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAgregar.Click
        ...
        If cbxDebeHaber.Text = "Debe" Then
            dgbParaAsentarDetalle.Rows(CantReg - 1).Cells("IMPORTED1").Value = tbxDebe.Text
            dgbParaAsentarDetalle.Rows(CantReg - 1).Cells("IMPORTEH1").Value = 0
        ElseIf cbxDebeHaber.Text = "Haber" Then
            dgbParaAsentarDetalle.Rows(CantReg - 1).Cells("IMPORTEH1").Value = tbxHaber.Text
        ...
        For i = 0 To dgbParaAsentarDetalle.RowCount - 1
            ...
            Else
                ...
                Try
                    ...
                    If dgvParaAsentar.CurrentRow.Cells("ModuloID").Value = 8 Then   'VENTAS
                        vlCodsucursal = f.FuncionConsultaDecimal("CODSUCURSAL", "VENTAS", "CODVENTA", dgvParaAsentar.CurrentRow.Cells("RegistroID").Value)

                    ElseIf dgvParaAsentar.CurrentRow.Cells("ModuloID").Value = 29 Then  'DEVOLUCION DE COMPRA
                        vlCodsucursal = f.FuncionConsultaDecimal("CODSUCURSAL", "DEVOLUCIONCOMPRA", "CODDEVOLUCION", dgvParaAsentar.CurrentRow.Cells("RegistroID").Value)

                    ElseIf dgvParaAsentar.CurrentRow.Cells("ModuloID").Value = 10 Then  'COBRO
                        Dim vCodVenta As Integer = f.FuncionConsultaDecimal("CODVENTA", "VENTASFORMACOBRO", "CODCOBRO", dgvParaAsentar.CurrentRow.Cells("RegistroID").Value)
                        vlCodsucursal = f.FuncionConsultaDecimal("CODSUCURSAL", "VENTAS", "CODVENTA", vCodVenta)
                    End If
...
#End Region
    ...
    Private Sub dgvParaAsentar_CellContentClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvParaAsentar.CellContentClick
