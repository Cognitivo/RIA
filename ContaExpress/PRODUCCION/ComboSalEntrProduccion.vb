...
Public Class ComboSalEntrProduccion
    ...
    Dim EntradaSalida, CantCombo As Integer
    Dim cambioCant, modifDet As Integer
    Dim vgCantAnterior As Integer = 0
    ...
    Private Sub ProdSaliaEntredaProduccion_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        permiso = PermisoAplicado(UsuCodigo, 249)
        If permiso = 0 Then
        ...
        Else
            ...
            Try
                Me.PRODENTRADASALIDATableAdapter.Fill(Me.DsProdSimple.PRODENTRADASALIDA, FechaFiltro1, FechaFiltro2, 2)
    ...
    Private Sub txtCantidad_GotFocus(sender As Object, e As System.EventArgs) Handles txtCantidad.GotFocus
        If cambioCant = 0 Then
            txtCantidad.Text = CantCombo
        End If
    End Sub
    ...
    Private Sub NuevoPictureBox_Click(sender As System.Object, e As System.EventArgs) Handles NuevoPictureBox.Click
        permiso = PermisoAplicado(UsuCodigo, 246)
        If permiso = 0 Then
        ...
        Dim MaxNro As Double = f.MaximoconWhere("NROPRODUCCION", "PRODENTRADASALIDA", "TIPO", 2)
        MaxNro = MaxNro + 1
        txtNroProduccion.Text = MaxNro
        txtCantidad.BackColor = Control.DefaultBackColor
    ...
    Private Sub btnAgregar_Click(sender As System.Object, e As System.EventArgs) Handles btnAgregar.Click
        ...
        If CantCombo = 0 Then 'Solo entrara la primera vez
            CantCombo = CInt(Me.txtCantidad.Text)
            dgwProduccionDet.Rows(c - 1).Cells("CANTIDADDataGridViewTextBoxColumn").Value = CDec(Me.txtCantidad.Text)
        ElseIf CInt(txtCantidad.Text) <> CantCombo Then
            Dim intAnswer As Integer
            intAnswer = MsgBox("Ha Cambiado la Cantidad de Entrada/Salida de Productos y éstas deben ser iguales. Desea Actualizarlas con la nueva Cantidad ingresada?", vbOKCancel, "Cantidad Combo")
            If intAnswer = vbOK Then
                For i = 0 To dgwProduccionDet.RowCount - 2 'Porque el ultimo no se inserto todavia
                    dgwProduccionDet.Rows(i).Cells("CANTIDADDataGridViewTextBoxColumn").Value = CDec(txtCantidad.Text)
                    dgwProduccionDet.Rows(i).Cells("EstadoGrilla").Value = "U"
                Next
                dgwProduccionDet.Rows(c - 1).Cells("CANTIDADDataGridViewTextBoxColumn").Value = CDec(Me.txtCantidad.Text)
                CantCombo = CInt(txtCantidad.Text)
                txtCantidad.BackColor = Control.DefaultBackColor
            Else
                cambioCant = 1
                txtCantidad.Focus()
                txtCantidad.BackColor = Color.Pink
                PRODENTRADASALIDADETBindingSource.CancelEdit()
                Exit Sub
            End If
        Else
            dgwProduccionDet.Rows(c - 1).Cells("CANTIDADDataGridViewTextBoxColumn").Value = CDec(Me.txtCantidad.Text)
    ...
    Private Sub ModificarPictureBox_Click(sender As System.Object, e As System.EventArgs) Handles ModificarPictureBox.Click
        permiso = PermisoAplicado(UsuCodigo, 247)
        If permiso = 0 Then
    ...
    Private Sub GuardarPictureBox_Click(sender As System.Object, e As System.EventArgs) Handles GuardarPictureBox.Click
        ...
        RecorreDetalle()
        Deshabilita()
        txtCantidad.Text = "" : vgCantAnterior = 0
    End Sub
        ...
        For c = 1 To c
            ...
            If dgwProduccionDet.Rows(c - 1).Cells("EstadoGrilla").Value = "I" Then
                Sql = "INSERT MOVPRODUCTO (CODUSUARIO,CODCODIGO,CODDEPOSITO,FECHAMOVIMIENTO,MODULO,MODULOTRANSID,CANTIDAD,VALOR,CONCEPTO,COSTEABLE,TIPOPRODUCCION,STOCK) " & _
                       ...
                       "," & CantidadFactura & "," & Precio & ",'Combos Nro " & txtNroProduccion.Text & "'," & Costeable & ",2,1)"
            ...
            ElseIf dgwProduccionDet.Rows(c - 1).Cells("EstadoGrilla").Value = "D" Then
                Sql = "DELETE FROM MOVPRODUCTO WHERE CODMOVIMIENTO=(SELECT MAX(CODMOVIMIENTO) FROM MOVPRODUCTO WHERE CODCODIGO = " & CodCodigoFactura & " AND MODULO  = 28 AND MODULOTRANSID= " & dgwProduccion.CurrentRow.Cells("CODPRODUCCIONDataGridViewTextBoxColumn").Value & _
                      " AND CANTIDAD = " & CantidadFactura & " AND TIPOPRODUCCION = 2)"
    ...
    Private Sub ActualizaCabecera()
        ...
        Sql = "UPDATE PRODENTRADASALIDA SET CODUSUARIO = " & UsuCodigo & ", CODDEPOSITO = " & cbxDeposito.SelectedValue & ",FECHA = CONVERT (DATETIME, '" & FechaCompleta & "',103), NROPRODUCCION = " & _
               Me.txtNroProduccion.Text & ",OBSERVACION = '" & Obs & "', TIPO = " & 2 & ",TOTALPRODENTRADA = " & TotalEntrada & ",TOTALPRODSALIDA = " & TotalSalida & "  " & _
    ...
    Private Sub btnAliminarProducto_Click(sender As System.Object, e As System.EventArgs) Handles btnEliminarProducto.Click
       Dim c As Integer = dgwProduccionDet.RowCount
    ...
    Private Sub EliminarPictureBox_Click(sender As System.Object, e As System.EventArgs) Handles EliminarPictureBox.Click
        permiso = PermisoAplicado(UsuCodigo, 248)
        If permiso = 0 Then
    ...
    Private Sub EliminaMovProducto()
        ...
        For i = 0 To c - 1
            ...
            Sql = "DELETE FROM MOVPRODUCTO WHERE CODMOVIMIENTO=(SELECT MAX(CODMOVIMIENTO) FROM MOVPRODUCTO WHERE MODULO  = 28 AND TIPOPRODUCCION = 2 AND CODCODIGO = " & CodCodigo & "AND MODULOTRANSID= " & dgwProduccion.CurrentRow.Cells("CODPRODUCCIONDataGridViewTextBoxColumn").Value & ")"
    ...
    Private Sub pbxImprimirOrden_Click(sender As System.Object, e As System.EventArgs) Handles pbxImprimirOrden.Click
        ...
        Me.RPTPRODUCCIONTableAdapter.Fill(DsProdSimple.RPTPRODUCCION, Me.lblNroProduccion.Text, 2)

        rptmat.SetDataSource([DsProdSimple])
        rptmat.SetParameterValue("pmtEmpresa", EmpDescripcion)
        rptmat.SetParameterValue("pmtComprobante", "Entrada / Salida de Combos")
    ...
    Private Sub btnModificarDet_Click(sender As System.Object, e As System.EventArgs) Handles btnModificarDet.Click
        ...
        If (tbxCodigoProducto.Text <> "") And (txtCantidad.Text <> "") And (tbxPrecio.Text <> "") Then
            If CInt(txtCantidad.Text) <> CantCombo Then
                Dim intAnswer As Integer
                intAnswer = MsgBox("Ha cambiado la cantidad de Entrada/Salida de Productos y éstas deben ser iguales. Desea Actualizarlas con la nueva Cantidad ingresada?", vbOKCancel, "Cantidad Salida Combos")
                If intAnswer = vbOK Then
                    For i = 0 To dgwProduccionDet.RowCount - 1
                        dgwProduccionDet.Rows(i).Cells("CANTIDADDataGridViewTextBoxColumn").Value = CDec(txtCantidad.Text)
                        dgwProduccionDet.Rows(i).Cells("ESTADOGRILLA").Value = "U"
    ...
    Private Sub dgwProduccionDet_DoubleClick(sender As Object, e As System.EventArgs) Handles dgwProduccionDet.DoubleClick
        Try
            ...
            Else
                ...
                txtCantidad.Text = CInt(dgwProduccionDet.CurrentRow.Cells("CANTIDADDataGridViewTextBoxColumn").Value)
                vgCantAnterior = txtCantidad.Text : cambioCant = 1
                ...
                vgCodCodigo = dgwProduccionDet.CurrentRow.Cells("CODCODIGODataGridViewTextBoxColumn").Value
                vgCodProducto = dgwProduccionDet.CurrentRow.Cells("CODPRODUCTODataGridViewTextBoxColumn").Value
            End If
    ...
    Private Sub dgwProduccionDet_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgwProduccionDet.CellContentClick
