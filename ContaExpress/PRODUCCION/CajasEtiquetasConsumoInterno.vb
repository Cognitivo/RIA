...
Public Class CajasEtiquetasConsumoInterno
    ...
    Private Sub ProdConsumoInterno_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        permiso = PermisoAplicado(UsuCodigo, 253)
        If permiso = 0 Then
        ...
        Else
            ...
            Me.PRODCONSUMOINTERNOTableAdapter.Fill(Me.DsProdSimple.PRODCONSUMOINTERNO, FechaFiltro1, FechaFiltro2, 5)
            Try
    ...
    Private Sub NuevoPictureBox_Click(sender As System.Object, e As System.EventArgs) Handles NuevoPictureBox.Click
        permiso = PermisoAplicado(UsuCodigo, 250)
        If permiso = 0 Then
        ...
        Dim MaxNro As Double = f.MaximoconWhere("NROCONSUMO", "PRODCONSUMOINTERNO", "TIPO", 5)
        MaxNro = MaxNro + 1
    ...
    Private Sub ModificarPictureBox_Click(sender As System.Object, e As System.EventArgs) Handles ModificarPictureBox.Click
        permiso = PermisoAplicado(UsuCodigo, 251)
        If permiso = 0 Then
        ...
        For c = 1 To c
            ...
            If dgwConsumoDet.Rows(c - 1).Cells("EstadoGrilla").Value = "I" Then
                Sql = "INSERT MOVPRODUCTO (CODUSUARIO,CODCODIGO,CODDEPOSITO,FECHAMOVIMIENTO,MODULO,MODULOTRANSID,CANTIDAD,VALOR,CONCEPTO,COSTEABLE,TIPOPRODUCCION,STOCK) " & _
                       ...
                       "," & CantidadFactura & "," & Precio & ",'Cajas y Etiquetas Nro " & txtNroConsumo.Text & "'," & Costeable & ",5,1)"
            ...
            ElseIf dgwConsumoDet.Rows(c - 1).Cells("EstadoGrilla").Value = "U" Then
                ...
                ''Aqui debemos saber que cantidad exactamente se agrego nueva
                'Dim CantNueva, CantAnterior, CantDif As Decimal

                'If IsDBNull(dgwConsumoDet.Rows(c - 1).Cells("CantAnterior").Value) Then
                '    CantAnterior = 0
                'Else
                ...
                'CantDif = CantAnterior - CantNueva

                'CantidadFactura = Funciones.Cadenas.QuitarCad(CantDif, ".")
                'CantidadFactura = Replace(CantidadFactura, ",", ".")
                ...
                Sql = "UPDATE MOVPRODUCTO SET CANTIDAD=" & CantNueva & ", CODCODIGO=" & CodCodigoFactura & ",VALOR=" & Precio & " WHERE CANTIDAD=" & CantAnterior & " AND CODCODIGO=" & dgwConsumoDet.Rows(c - 1).Cells("CodCodigoAnt").Value & " AND CODDEPOSITO=" & cbxDeposito.SelectedValue & " AND MODULOTRANSID=" & dgwComsumo.CurrentRow.Cells("CODCONSUMODataGridViewTextBoxColumn").Value & " AND MODULO=28 AND TIPOPRODUCCION=5"
            ...
            ElseIf dgwConsumoDet.Rows(c - 1).Cells("EstadoGrilla").Value = "D" Then
                ...
                Sql = "DELETE FROM MOVPRODUCTO WHERE CODMOVIMIENTO=(SELECT MAX(CODMOVIMIENTO) FROM MOVPRODUCTO WHERE CODCODIGO = " & CodCodigoFactura & " AND MODULO  = 28 AND MODULOTRANSID= " & dgwComsumo.CurrentRow.Cells("CODCONSUMODataGridViewTextBoxColumn").Value & _
                 " AND CANTIDAD = " & CantidadFactura & " AND TIPOPRODUCCION = 5)"
    ...
    Private Sub ActualizaCabecera()
        ...
        Sql = "UPDATE PRODCONSUMOINTERNO SET OBSERVACION = '" & Obs & "',TOTALCONSUMO = " & TotalConsumo & ",FECHA= CONVERT (DATETIME, '" & Me.dtpFecha.Text & "',103) , NROCONSUMO = " & txtNroConsumo.Text & ", TIPO= " & 5 & "   " & _
    ...
    Private Sub InsertaCabecera()
        ...
        Sql = "INSERT INTO PRODCONSUMOINTERNO (CODUSUARIO ,CODDEPOSITO,NROCONSUMO,OBSERVACION,TOTALCONSUMO,FECHA,TIPO)  " & _
              "VALUES(" & UsuCodigo & "," & cbxDeposito.SelectedValue & "," & txtNroConsumo.Text & ",'" & Obs & "'," & TotalConsumo & ", CONVERT (DATETIME, '" & Me.dtpFecha.Text & "',103), " & 5 & ")   " & _
    ...
    Private Sub EliminarPictureBox_Click(sender As System.Object, e As System.EventArgs) Handles EliminarPictureBox.Click
        permiso = PermisoAplicado(UsuCodigo, 252)
        If permiso = 0 Then
    ...
    Private Sub EliminaMovProducto()
        ...
        For i = 0 To c - 1
            ...
            Sql = "DELETE FROM MOVPRODUCTO WHERE CODMOVIMIENTO=(SELECT MAX(CODMOVIMIENTO) FROM MOVPRODUCTO WHERE TIPOPRODUCCION = 5 AND MODULO  = 28 AND MODULOTRANSID= " & dgwComsumo.CurrentRow.Cells("CODCONSUMODataGridViewTextBoxColumn").Value & ")"
    ...
    Private Sub pbxImprimirOrden_Click(sender As System.Object, e As System.EventArgs) Handles pbxImprimirOrden.Click
        ...
        Me.RPTCONSUMOINTERNOTableAdapter.Fill(DsProdSimple.RPTCONSUMOINTERNO, Me.lblNroConsumo.Text, 5)

        rptmat.SetDataSource([DsProdSimple])
        rptmat.SetParameterValue("pmtEmpresa", EmpDescripcion)
        rptmat.SetParameterValue("pmtComprobante", "Salida de Cajas, Envases y Etiquetas")
    ...
    Private Sub btnModificarDet_Click(sender As System.Object, e As System.EventArgs) Handles btnModificarDet.Click
        ...
        If (tbxCodigoProducto.Text <> "") And (txtCantidad.Text <> "") And (tbxPrecio.Text <> "") Then
            dgwConsumoDet.Rows(vgLineaConsumo).Cells("CODPRODUCTO2").Value = vgCodCodigo
